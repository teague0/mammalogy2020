...
#### Richness
First, we'll calculate a very simple **Richness** calcualtion for each Subproject / Forest Type 
```{r}
rich <- cam %>% group_by(Subproject) %>% 
  filter(!is.na(Species.Name)) %>%  #filter out any NAs in the species name, just in case
  summarize(ForestRichness = length(unique(Species.Name)))
rich
```
